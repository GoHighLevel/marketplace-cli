...
describe('workflow action guide', () => {
  it('documents every supported JSON key and the execution-test command', () => {
    ...
    expect(guide).toContain('## Toolchain integration')
    expect(guide).toContain('tsconfig.actions.json')
    expect(guide).toContain('eslint.config.actions.mjs')
    expect(guide).toContain('// @ts-check')
  })
})
