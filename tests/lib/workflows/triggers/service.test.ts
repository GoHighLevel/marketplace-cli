...
import { type WorkflowTriggerSummary } from '../../../../src/lib/api/client.js'
import {
  ...
  type WorkflowTriggersApi
} from '../../../../src/lib/workflows/triggers/service.js'
import { type WorkflowTriggersSyncPlan } from '../../../../src/lib/workflows/triggers/sync.js'
...
describe('workflow trigger service orchestration', () => {
  ...
  it('fails closed for duplicate or cross-app workflow configuration', async () => {
    ...
    ).rejects.toThrow(/duplicate template-1:1\.0/i)
    ...
    ).rejects.toThrow(/belongs to app "app-2"/i)
  })

  it('redacts local headers while retaining runtime values for safe preservation', async () => {
    const snapshot = await fetchWorkflowTriggersSnapshot(
      api({
        listWorkflowTriggerConfigs: vi.fn().mockResolvedValue([
          config({
            subscriptionConfig: {
  ...
  it('preflights key availability and all secret references before any mutation', async () => {
    ...
    await expect(executeWorkflowTriggersSyncPlanIndependently(client, plan, { environment: {} })).rejects.toThrow(
      /MISSING_TOKEN/
    )
  ...
  it('updates only changed trigger configuration and registry name', async () => {
    ...
    const plan: WorkflowTriggersSyncPlan = {
      appId: 'app-1',
      localChanges: ['triggers.contact_changed.versions.1.0.info.name'],
      remoteChanges: [],
      conflicts: [],
      errors: [],
      operations: [
  ...
  it('hydrates remote-preservation references from unredacted runtime state', async () => {
    ...
    const desired = {
      ...
      info: { name: 'Contact changed', description: 'Updated' },
      subscriptionConfig: { url: 'https://example.com/subscriptions', headers: { Authorization: '${remote}' } }
    }
    const plan: WorkflowTriggersSyncPlan = {
      appId: 'app-1',
      localChanges: ['triggers.contact_changed.versions.1.0.info.description'],
      remoteChanges: [],
      conflicts: [],
      errors: [],
      operations: [
    ...
    expect(client.updateWorkflowTriggerConfig).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        subscriptionConfig: expect.objectContaining({ headers: { Authorization: 'Bearer actual' } })
      })
    )
  })
      ...
      triggers: [
        {
          key: 'contact_changed',
          versions: [
            { version: '1.0', status: 'draft' as const, info: { name: 'Contact changed', description: 'Retry' } }
          ]
        }
      ]
    }
  ...
  it('finds drafts and registry repairs as publication candidates', () => {
    ...
    expect(
      workflowTriggerPublishCandidates(draftManifest, [{ ...summary(), status: 'approved', isActive: true }])
    ).toEqual([])
  })
