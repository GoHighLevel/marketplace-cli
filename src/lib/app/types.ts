...
import { generatedFileHeader } from '../shared/generated-file.js'
import {
  getJsonSchema,
  type JsonSchema,
  type JsonSchemaName,
...
export async function writeTypeDeclarationFile(
  ...
  await writeTextFileAtomic(outputFile, generateTypeDeclarations(), 0o644)
  return outputFile
}
...
export async function writeTypesWorkspace(
  ...
  const actionTypes =
    actions && actions.manifest.actions.length > 0
      ? await writeWorkflowActionTypesWorkspace(directory, actions.manifest)
      : undefined
