...
export default class AppPricingSetup extends GhlCommand {
  ...
  protected async execute(): Promise<void> {
    ...
    try {
      ...
      this.error(errorMessage(error, 'Invalid pricing configuration'))
    }
    ...
    try {
      ...
      const message = errorMessage(error, 'Pricing configuration could not be saved.')
      if (deletedFreePlans) {
        throw new Error(
